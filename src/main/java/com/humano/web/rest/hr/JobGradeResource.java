...
import com.humano.service.hr.JobGradeService;
...
/**
 * REST controller for managing JobGrade reference data.
 */
@RestController
@RequestMapping("/api/hr/job-grades")
public class JobGradeResource {

    private static final Logger LOG = LoggerFactory.getLogger(JobGradeResource.class);
    ...
    public JobGradeResource(JobGradeService service) {
    ...
    public ResponseEntity<ReferenceDataResponse> create(@Valid @RequestBody CreateReferenceDataRequest request) throws URISyntaxException {
        LOG.debug("REST request to create JobGrade: {}", request);
        ReferenceDataResponse result = service.create(request);
        return ResponseEntity.created(new URI("/api/hr/job-grades/" + result.id()))
    ...
    public ResponseEntity<ReferenceDataResponse> update(@PathVariable UUID id, @Valid @RequestBody UpdateReferenceDataRequest request) {
        LOG.debug("REST request to update JobGrade: {}", id);
